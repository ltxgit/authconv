...
输入文本支持单个 JSON、JSONL，以及连续拼接的多个完整 JSON 文档。输入路径还支持 `.zip` 文件；目录输入只读取目录一级的 `.json` / `.jsonl` / `.zip` 文件，不递归。ZIP 会在本地解压，只读取其中的 `.json` / `.jsonl` 文件，并忽略 `__MACOSX` 和隐藏路径段。
...
| codex2api | 自动识别单账号对象时要求是扁平 token 对象，并且至少有 `refresh_token`、`session_token` 或 `id_token` 之一；不能带 `type`、`credentials`、`tokens`、`accounts` 等其他格式特征。手动指定输入格式时可从任意 token 字段抽取账号。 |
...
多个输入文件里出现相同账号时会去重。判断只使用 `access_token`、`refresh_token`、`session_token` 和真实 `id_token`。同一字段两边都有值时必须相等；一边缺失时不算冲突，后续重复项会补齐缺失字段。两条记录至少要有一个重叠且相等的凭据字段才会合并，不会仅凭邮箱、账号 ID 或其他元数据猜测合并。
