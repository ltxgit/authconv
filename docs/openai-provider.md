...
本文档拥有 OpenAI、ChatGPT 与 Codex 专属的输入识别、JWT claims、字段优先级、synthetic `id_token` 和输出字段规则。通用解析、provider 冲突、去重、格式适用性和文件命名见 [conversion-rules.md](conversion-rules.md)。
...
Session / Codex 输入发现 JSON 与 `access_token` claims 不一致时，会按来源聚合 warning，并列出被覆盖字段。
...
`https://api.openai.com/auth` 是 JWT claim 名，不是请求地址。JWT 只在本地解码 payload，不联网、不验签。

OpenAI 会检查 issuer、audience 和 `nbf <= exp`。audience 存在时应包含 `https://api.openai.com/v1`。异常只产生 warning；凭证已经过期只是状态，不产生 warning。
